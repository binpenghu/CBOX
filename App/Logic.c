...
u8 judgeBatType(u16 batval)
{
	...
	for (i=0;i<BAT_MAX;i++)
	{
		if (batval <= (batTable[i].batHighVolt+10) && batval >= batTable[i].batLowVolt)
		{
			return i;
		}
	}
...
u8 judgeBatStatus(u16 batval)
{
	...
	if (batval >= 10)// >1V
	{
		...
		{
			if (batval <= (batTable[batInfo.batType].batHighVolt+10) && batval >= batTable[batInfo.batType].batLowVolt)
			{
				return BAT_IS_NORMAL;
			}
			//else 
...
u16 getChargePercent(BAT_INFO_T *bat)
{
	...
	{
		percent = (u16)(((float)(curVolt-minVolt)/(float)(maxVolt-minVolt))*1000); // 0.1%
	}
	else if (curVolt> maxVolt)
	{
		percent = 1000;
	}
	//percent = 1000; //for test
	return percent;
}
...
void logicTask(void)
{
	...
	if (batRtn.curSta != batRtn.oldSta && vBatBufSta.bufSta == 1)
	{
	...
	//charge full
	if (batInfo.batVoltage>= batTable[batInfo.batType].batHighVolt 
		&& batInfo.batChargeSta == 0 )//&& inRtn.curSta == IN_CHARGE)
	{
...
void adDetectTask(void)
{
	...
	switch (step)
	{
		...
		case V_IN_STEP:
			...
			inVal = vInConvert(temp);
			if (0 == buffer(vInBuf,&vInBufSta,inVal))
			{
				batInfo.InVolt = vInBufSta.avg;
			}
			step++;
			break;
		case I_BACK_STEP:
			temp = adcSingleRead(I_BACK_CHN);
			inVal = iBackConvert(temp);
			if (0 == buffer(iBatBuf,&iBatBufSta,inVal))
			{
				batInfo.batCurrent = iBatBufSta.avg;
			}
			step++;
			break;
		case I_USB_STEP:
			temp = adcSingleRead(I_USB_CHN);
			inVal = iUsbConvert(temp);
			if (0 == buffer(iUsbBuf,&iUsbBufSta,inVal))
			{
				batInfo.usbCurr = iUsbBufSta.avg;
			}
			step=V_BACK_STEP;
			break;
		default:
